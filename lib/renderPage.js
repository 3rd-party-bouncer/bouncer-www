  ...
  if ( !cache.templateStrings[ templateId ] ) {
    ...
    fs.readFile( templatePath, 'utf8', function ( err, templateString) {
      ...
      if ( err ){
        ...
        page = '<script type="text/template" id="dataTable"><div><dd><strong>allowedUrls:</strong> <%= result.allowedUrls %> <br><strong>blockedUrls:</strong> <%= result.blockedUrls %> <br><strong>firstView</strong><br><br><table><thead><tr><th>loadEnd</th><th>loadTime</th><th>requests</th>       </tr>     </thead>     <tbody>       <tr><td> <%= result.firstView.loadEnd %> </td><td> <%= result.firstView.loadTime %> </td><td> <%= result.firstView.requests %> </td>       </tr>     </tbody>   </table>   <strong>repeatView</strong>   <table>     <thead>       <tr><th>loadEnd</th><th>loadTime</th><th>requests</th>       </tr>     </thead>     <tbody>       <tr><td> <%= result.repeatView.loadEnd %> </td><td> <%= result.repeatView.loadTime %> </td><td> <%= result.repeatView.requests %> </td>       </tr>     </tbody>   </table> </dd> </div></script>' + _.template(
          cache.layout,
          {
            content: content
          }
    ...
    ) + '<script type="text/template" id="dataTable"> <dd><strong>allowedUrls:</strong> <%= result.allowedUrls %> <br><strong>blockedUrls:</strong> <%= result.blockedUrls %> <br><strong>firstView</strong><br><br><table><thead><tr><th>loadEnd</th><th>loadTime</th><th>requests</th>       </tr>     </thead>     <tbody>       <tr><td> <%= result.firstView.loadEnd %> </td><td> <%= result.firstView.loadTime %> </td><td> <%= result.firstView.requests %> </td>       </tr>     </tbody>   </table>   <strong>repeatView</strong>   <table>     <thead>       <tr><th>loadEnd</th><th>loadTime</th><th>requests</th>       </tr>     </thead>     <tbody>       <tr><td> <%= result.repeatView.loadEnd %> </td><td> <%= result.repeatView.loadTime %> </td><td> <%= result.repeatView.requests %> </td>       </tr>     </tbody>   </table> </dd></script>';
